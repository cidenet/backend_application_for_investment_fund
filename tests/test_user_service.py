...
class TestUserService(unittest.TestCase):
    ...
    def test_create_user_success_with_uuid(self, mock_db):
        user = User(name="John Doe", email="john@example.com", investment_capital=1000.0)
    ...
    @patch('app.services.user_service.db')
    def test_create_user_success_with_uuid_and_investment_capital_is_None(self, mock_db):
    ...
    # @patch('app.services.user_service.db')
    # def test_create_user_db_error(self, mock_db):
    #     mock_user = User(name="Test User", email="test@example.com", investment_capital=None)
    #     mock_db.users.insert_one.side_effect = PyMongoError("Database error")
    ...
    #     self.assertIn("An error occurred while creating the user", context.exception.detail)
    ...
    def test_get_all_users_success(self, mock_db):
        mock_db.users.find.return_value = [
            {"id": "1", "name": "User1", "email": "user1@example.com", "investment_capital": 1000.0},
            {"id": "2", "name": "User2", "email": "user2@example.com", "investment_capital": 2000.0}
        ]
    ...
    def test_get_all_users_db_error(self, mock_db):
        ...
        self.assertEqual(context.exception.status_code, 500)
        self.assertIn("An error occurred while fetching users", context.exception.detail)
