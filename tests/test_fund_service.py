...
from app.utils.constants import DataBaseError, FundSuccessMessage
...
def test_list_funds(fund_service, mock_repository):
    mock_repository.list_funds.return_value = [
        {"id": "1", "name": "test", "minimum_investment_amount": 100000, "category": "FPV"}
    ]
...
def test_create_fund_Http_exception_error_v3(fund_service, mock_repository):
    ...
    mock_repository.create_fund.side_effect = HTTPException(status_code=400, detail="Bad Request")
